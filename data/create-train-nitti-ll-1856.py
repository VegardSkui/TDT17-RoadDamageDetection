...
OUTPUT_DIR = pathlib.Path("./derived/train-nitti-ll-1856")
...
def main():
    ...
    for xml_path in tqdm.tqdm(INPUT_DIR.joinpath("annotations/xmls").glob("*.xml"), desc="process annotations"):
        ...
        for object in annotation.iter("object"):
            ...
            # within the 1856x1856 box in the lower left corner of the image
            # The y-coordinates are adjusted to be relative to the cropped image
            # (we can assume the image to be taller than 1856px)
            bndbox = object.find("bndbox")
            xmin = min(float(bndbox.findtext("xmin")), 1856)
            ymin = max(float(bndbox.findtext("ymin")) - (image_height - 1856), 0)
            xmax = min(float(bndbox.findtext("xmax")), 1856)
            ymax = max(float(bndbox.findtext("ymax")) - (image_height - 1856), 0)

            # Skip the box if the remaining area is zero
            if (xmax - xmin) * (ymax - ymin) == 0:
                continue

            x_center = (xmax + xmin) / 2 / 1856
            y_center = (ymax + ymin) / 2 / 1856
            width = (xmax - xmin) / 1856
            height = (ymax - ymin) / 1856
            boxes.append([NAMES.index(name), x_center, y_center, width, height])

        # Keep the name and annotations if there is at least one object or with
        # random chance of 1%
        name = xml_path.name[:-4]
        if len(boxes) > 0 or random.random() <= 0.01:
